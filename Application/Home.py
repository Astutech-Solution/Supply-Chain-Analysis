...
if selected=='Home':
    ...
    if file is not None:
        ...
        upper=pd.to_datetime("12/31/2024")
        df1= rej_df.loc[(rej_df['VENDOR_ID']==inp1)&(rej_df['ITEM_ID'].isin(inp2)) &(rej_df['TRANSACTION_DATE']<=upper)].sort_values(by=['ITEM_ID','TRANSACTION_DATE','REJECTION_RATE'])
        ...
        if search:
            ...
            df1= rej_df.loc[(rej_df['VENDOR_ID']==inp1) & (rej_df['ITEM_ID'].isin(inp2)) &(rej_df['TRANSACTION_DATE']>=date_1) &(rej_df['TRANSACTION_DATE']<=date_2) &(rej_df['TRANSACTION_DATE']<=upper) ].sort_values(by=['ITEM_ID','TRANSACTION_DATE','REJECTION_RATE'])
